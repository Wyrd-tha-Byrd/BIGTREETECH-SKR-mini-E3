...
var WmButtonGroups = {
  ...
  All: function All() {
    var all = [];
    all = all.concat(WmButtonGroups.FileManagement, WmButtonGroups.FileActions, WmButtonGroups.FileProcess, WmButtonGroups.TempStatus, WmButtonGroups.FanSpeed, WmButtonGroups.MoveHome, WmButtonGroups.Move, WmButtonGroups.StepperAll);
    return all;
  }
};
var wmWebSoket = {
  ...
  Connect: function Connect() {
    WsUrl = "ws://".concat(location.host, "/ws");
  ...
  Send: function Send(gCmd) {
    if (wmWebSoket.WSObject !== null && wmWebSoket.WSObject.readyState === wmEnums.WSSatuses.OPEN) {
      var strcmd = gCmd.GCode;

      if (gCmd.GParams === null) {
        jsLog.Verbose('WebSocket: Send: Command with no params detected');
      } else {
      ...
      try {
        wmWebSoket.WSObject.send(strcmd + '\n');
      } catch (exception) {
        jsLog.Error('WebSocket: Exception:' + exception);
  ...
  OnMessage: function OnMessage(mdt) {
    if (mdt === "") {
      jsLog.Debug("WSMessage match: Empty message (skipped)");
    } else {
      var litem = wmLogItem.ParseWsMessage(mdt);
      WmConsole.Trace(litem);

      if (litem.SdFile !== null) {
        WmConsole.TraceSdFile(litem);
      }
    }
  },
  SetWsStatusBar: function SetWsStatusBar(rs) {
    var cli = new wmLogItem();
    var ctrls = {
      div: $('#div-conn-statusmsg'),
      ico: $('#div-conn-statusico'),
      btn: $('#btn-connect-status'),
      bar: $('#div-conn-statusbar')
    };
...
var WmUpload = {
  ...
  Cancelled: false,
  Load: function Load() {
    var _$$text$toUpperCase$s = $("#file-upload-label").text().toUpperCase().split("."),
        _$$text$toUpperCase$s2 = _slicedToArray(_$$text$toUpperCase$s, 2),
        fn = _$$text$toUpperCase$s2[0],
        fe = _$$text$toUpperCase$s2[1];

    WmUpload.FileName = fn.substring(0, 7) + ".GCO";
    var input = $('#file-upload').get(0);

    if (input.files.length) {
      var tfile = input.files[0];
      ...
      WmUpload.Reader.onloadstart = function () {
        ...
        if (!$('#upload-process-collapse').hasClass("show")) {
          $('#upload-process-collapse').collapse('show');
        }
      };
  ...
  Cancel: function Cancel() {
    ...
    if ($('#upload-process-collapse').hasClass("show")) {
      $('#upload-process-collapse').collapse('hide');
    }
  },
  FileProcess: function FileProcess() {
    WmControls.Disable([$("#btn-file-proc")]);
    var fl = WmUpload.FileContent.length;
    ...
    (function pgline() {
      if (WmUpload.Cancelled) {
        ...
        var p = wmTools.GetPercentage(i + 1, fl);
        WmUpload.FileProgress(p, "Analyzing line " + (i + 1) + " of " + fl);
        var gitem = {
          line: WmUpload.FileContent[i],
          process: false,
          cksum: 0
        };
  ...
  FileProgress: function FileProgress(p, m) {
    ...
    if (p === 100) {
      $('#upload-progressbar').removeClass("progress-bar-animated");
    }
  },
  FileCompleted: function FileCompleted() {
    WmControls.Disable(WmButtonGroups.FileProcess);
    WmButtons.GetSdContentList();
  }
};
var WmButtons = {
  ConsoleListClear: function ConsoleListClear() {
    WmConsole.Clear();
  },
  ConsoleListExport: function ConsoleListExport() {
    WmConsole.Export();
  },
  DeleteSdConfirm: function DeleteSdConfirm() {
    $('#div-sdfile-delete-badge').html($('#txt-sdfile-selected').val());
  },
  DeleteSdSelected: function DeleteSdSelected() {
  ...
  PrintSdConfirm: function PrintSdConfirm() {
    $('#div-sdfile-print-badge').html($('#txt-sdfile-selected').val());
  },
  PrintSdSelected: function PrintSdSelected() {
  ...
  SaveSettings: function SaveSettings() {
    ...
    if ($('#set-default-tempunit').val() === 0) {
      wmSettings.TempUnit = wmEnums.TempUnits.CELSIUS;
    } else if ($('#set-default-tempunit').val() === 1) {
      wmSettings.TempUnit = wmEnums.TempUnits.FAHRENHEIT;
    } else if ($('#set-default-tempunit').val() === 2) {
      wmSettings.TempUnit = wmEnums.TempUnits.KELVIN;
    }
  ...
  SdInit: function SdInit() {
    wmWebSoket.Send(wmGCommands.SdInit);
  },
  SdRelease: function SdRelease() {
    wmWebSoket.Send(wmGCommands.SdRelease);
  },
  SendGcommand: function SendGcommand() {
    if (wmWebSoket.WSObject !== null && wmWebSoket.WSObject.readyState === wmEnums.WSSatuses.OPEN) {
      WmControls.Disable(["#btn-gcommand"]);
      var gcmd = $('#text-gcommand');
      ...
      if (gcmd.val() === "") {
        ...
        var gc = wmGCommandItem.GetCommandItemByCode(gcmd.val().trim().toUpperCase());
  ...
  SetPositionHome: function SetPositionHome(b) {
    if (b.id === "btn-move-home-all") {
      wmWebSoket.Send(wmGCommands.MoveHome);
    } else if (b.id === "btn-move-home-x") {
      wmWebSoket.Send(wmGCommands.MoveHomeX);
    } else if (b.id === "btn-move-home-y") {
      wmWebSoket.Send(wmGCommands.MoveHomeY);
    } else if (b.id === "btn-move-home-z") {
      wmWebSoket.Send(wmGCommands.MoveHomeZ);
    }
  },
  SetSdSelected: function SetSdSelected(sdi) {
    jsLog.Debug("SetSdSelected: Selected file:" + $(sdi).attr("data-sdfile"));
    $('#txt-sdfile-selected').val($(sdi).attr("data-sdfile"));
    var lip = document.getElementById("list-sd-content").getElementsByTagName("a");

    for (var _i2 = 0; _i2 < lip.length; _i2++) {
      $(lip[_i2]).removeClass('list-group-item-success').addClass("list-group-item-light");
    }

    $(sdi).removeClass('list-group-item-light').addClass("list-group-item-success");
    WmControls.Enable(WmButtonGroups.FileActions);

    if (!$('#div-sd-selected-file').hasClass("show")) {
      $('#div-sd-selected-file').collapse("show");
    }

    wmGCommands.SdFileSel.GParams = $(sdi).attr("data-sdfile");
    wmWebSoket.Send(wmGCommands.SdFileSel);
  },
  ToggleChecksumDiv: function ToggleChecksumDiv() {
    var csdiv = $('#checksum-gcommand-div');

    if ($('#btn-gcommand-checksum').hasClass('active') === true) {
      csdiv.collapse('hide');
    } else {
      csdiv.collapse('show');
    }
  },
  UploadSdFile: function UploadSdFile() {
    WmControls.Disable(WmButtonGroups.FileManagement);
    WmControls.Disable(WmButtonGroups.FileActions);
  ...
  WsConnect: function WsConnect() {
    if (wmWebSoket.WSObject === null) {
      ...
      if (wmWebSoket.WSObject.readyState === wmEnums.WSSatuses.OPEN) {
        wmWebSoket.Disconnect();
      }
    }
  }
};
var WmControls = {
  Enable: function Enable() {
    if (arguments.length > 0) {
      for (var _i3 = 0; _i3 < arguments.length; _i3++) {
        var fld = arguments[_i3];

        for (var c = 0; c < fld.length; c++) {
          WmControls.SetInputStatus(fld[c], 'enabled');
        }
      }
    } else {
      jsLog.Warning("WmControls.Enable: Missing input arguments");
    }
  },
  Disable: function Disable() {
    if (arguments.length > 0) {
      for (var _i4 = 0; _i4 < arguments.length; _i4++) {
        var fld = arguments[_i4];

        for (var c = 0; c < fld.length; c++) {
          WmControls.SetInputStatus(fld[c], 'disabled');
        }
      }
    } else {
      jsLog.Warning("WmControls.Disable: Missing input arguments");
    }
  },
  SetInputStatus: function SetInputStatus(inid, st) {
    if ($(inid).attr("data-input-type") === "togglebtn") {
      if (st === "enabled") {
        $(inid).bootstrapToggle('enable');
      } else {
        $(inid).bootstrapToggle('disable');
      }
    } else {
      if (st === "enabled") {
  ...
  SetCheckStatus: function SetCheckStatus(ao, b) {
    var cs = "off";
    ...
    for (var _i5 = 0; _i5 < ao.length; _i5++) {
      $(ao[_i5]).bootstrapToggle(cs);
    }
  },
  ...
  ShowModalAlert: function ShowModalAlert(m) {
    $('#div-alert-message').html(m);
    $('#modal-alert').modal('show');
  },
  SetAutoTemp: function SetAutoTemp() {
    if ($('#set-auto-temp').prop('checked')) {
      wmGCommands.SetTempOn.GParams = "S" + $('#auto-temp-interval').val();
      wmWebSoket.Send(wmGCommands.SetTempOn);
    } else {
      wmWebSoket.Send(wmGCommands.SetTempOff);
    }
  },
  SetFanSpeed: function SetFanSpeed(rv) {
    rv = parseInt(rv);
    jsLog.Debug("Set fan speed to: " + rv + "%");
    var fsv = wmTools.FormatNumber(wmTools.GetNumPercent(rv, 255), 0);
    ...
    if (rv === 0) {
      wmWebSoket.Send(wmGCommands.FanOff);
    } else {
  ...
  ResetSdFileCounters: function ResetSdFileCounters() {
    $('#list-sd-content').empty();
    $('#div-sdlist-file-count').text(0);
    $('#div-sdlist-folder-count').text(0);
  },
  UpdateTemperatures: function UpdateTemperatures(dt) {
  ...
  SetPrinterStatusInfo: function SetPrinterStatusInfo(s, m) {
    if (!s) {
  ...
  SetSteppers: function SetSteppers(o) {
    if (o.id === "set-stepper-all") {
      WmControls.SetCheckStatus(WmButtonGroups.Stepper, o.checked);

      if (o.checked) {
        wmWebSoket.Send(wmGCommands.StepEnableAll);
      } else {
        wmWebSoket.Send(wmGCommands.StepDisableAll);
      }
    } else {
      if (o.id === "set-stepper-x") {
        wmGCommands.StepEnable.GParams = "X";
      } else if (o.id === "set-stepper-y") {
        wmGCommands.StepEnable.GParams = "Y";
      } else if (o.id === "set-stepper-z") {
        wmGCommands.StepEnable.GParams = "Z";
      } else if (o.id === "set-stepper-e") {
        wmGCommands.StepEnable.GParams = "E";
      }

      if (o.checked) {
        wmWebSoket.Send(wmGCommands.StepEnable);
      } else {
        wmWebSoket.Send(wmGCommands.StepDisable);
      }
    }
  }
};
var WmChartsData = {
  Temperatures: {
    ...
    AddEmpty: function AddEmpty(arr, n) {
      for (var i = 0; i < n; i++) {
        var xd = moment().subtract((n - i) * WmChartsData.Temperatures.Speed, 'milliseconds').toDate();
        arr.push({
          x: xd,
          y: null
        });
    ...
    DataUpdate: function DataUpdate(di) {
      var dt = new Date();
      WmChartsData.Temperatures.Extruder.Data.push({
        x: dt,
        y: di.ExtruderTemp
      });
      ...
      if (di.BedTemp !== null) {
        WmChartsData.Temperatures.Bed.Data.push({
          x: dt,
          y: di.BedTemp
        });
  ...
  FanSpeed: {
    ...
    DataUpdate: function DataUpdate(di) {
      ...
      if (di < 30) {
        WmChartsData.FanSpeed.Values.BgColor[0] = wmColors.GreenSuc;
      } else if (di < 60) {
        WmChartsData.FanSpeed.Values.BgColor[0] = wmColors.Yellow;
      } else if (di < 80) {
        WmChartsData.FanSpeed.Values.BgColor[0] = wmColors.Orange;
      } else {
        WmChartsData.FanSpeed.Values.BgColor[0] = wmColors.Red;
      }

      requestAnimationFrame(function () {
        WmCharts.FanSpeed.CanvasItem.update();
      });
    }
  }
};
var WmCharts = {
  Temperatures: {
    ...
    Config: {
      ...
      options: {
        responsive: true,
        animation: {
          duration: WmChartsData.Temperatures.Speed * 1.5,
          easing: 'linear'
        },
        scales: {
          xAxes: [{
    ...
    DisplayAxis: function DisplayAxis(ck) {
      ...
      options: {
        ...
        animation: {
          ...
          onComplete: function onComplete() {
            ...
            var model = ds[0]._meta[Object.keys(ds[0]._meta)[0]].data[0]._model;

            var xp = wmTools.FormatNumber(model.x, 0),
                yp = wmTools.FormatNumber(model.y, 0);
  ...
  Advance: function Advance() {
    if (WmChartsData.Temperatures.Extruder.Data[0] !== null && WmChartsData.Temperatures.Extruder.Scale < 4) {
      WmCharts.Temperatures.CanvasItem.update();
    }

    WmCharts.Temperatures.CanvasItem.update();
  },
  SetTempReport: function SetTempReport(tr) {
    var temps = {
...
var WmConsole = {
  ...
  Export: function Export() {
    ...
    var fdt = new Array(); // DA FINIRE LA PRIMA RIGA NOMI CAMPI

    for (i = 0; i < wmLogBuffer.length; i++) {
      fdt.push(wmLogBuffer[i].ToCsv() + "\n");
    }

    wmTools.FileDownload("e4dbox_log.csv", "text/csv;charset=utf-8", fdt);
  },
  SetMessageSymbol: function SetMessageSymbol() {
  ...
  SetSymbolIcon: function SetSymbolIcon() {
    var ctrl = parseInt($('#set-log-symbol-icon').val());
    var hc = {
      s: null,
      r: null
    };
    ...
    if (ctrl === 0) {
      hc.s = wmIcons.ChevronUp.ToHtml();
      hc.r = wmIcons.ChevronDown.ToHtml();
    } else if (ctrl === 1) {
      hc.s = wmIcons.LongArrowUp.ToHtml();
      hc.r = wmIcons.LongArrowDown.ToHtml();
    } else if (ctrl === 2) {
      hc.s = wmIcons.ChevronLeft.ToHtml();
      hc.r = wmIcons.ChevronRight.ToHtml();
    } else if (ctrl === 3) {
      hc.s = wmIcons.LongArrowLeft.ToHtml();
      hc.r = wmIcons.LongArrowRight.ToHtml();
    }

    $('#div-log-symbol-icon-sample-s').html(hc.s);
    $('#div-log-symbol-icon-sample-r').html(hc.r);
  },
  Trace: function Trace(litem) {
    wmLogBuffer.push(litem);
    var clist = $('#gcommand-console-list');

    if (wmSettings.ConsoleDirection === wmEnums.ConsoleDirection.APPEND) {
      clist.append(litem.ToLoglist());
    } else {
      clist.prepend(litem.ToLoglist());
    }

    $('#log-counter-badge').text(wmLogBuffer.length);
  },
  TraceSdFile: function TraceSdFile(litem) {
    if (litem.SdFile === "Begin file list") {
      wmSdListCounter.Reset();
    }
    ...
    if (litem.SdFile !== "Begin file list" && litem.SdFile !== "End file list") {
      ...
      if (litem.SdFile.indexOf("/") > -1) {
        wmSdListCounter.FOLDERS++;
      }
    }
...
var WmAutostart = {
  SetDefaultPanel: function SetDefaultPanel() {
    ...
    if (wmSettings.DefaultPanel == wmEnums.Panels.STATUS) {
      $('#accordion-panel-status-body').addClass("show");
    } else if (wmSettings.DefaultPanel === wmEnums.Panels.CONTROLS) {
      $('#accordion-panel-controls-body').addClass("show");
    } else if (wmSettings.DefaultPanel === wmEnums.Panels.TEMP) {
      $('#accordion-panel-temp-body').addClass("show");
    } else if (wmSettings.DefaultPanel === wmEnums.Panels.FILES) {
      $('#accordion-panel-file-body').addClass("show");
    } else if (wmSettings.DefaultPanel === wmEnums.Panels.CONSOLE) {
      $('#accordion-panel-console-body').addClass("show");
    }
  },
  SetShownPanel: function SetShownPanel(p) {
    jsLog.Verbose("Set shown panel (" + p + ")");

    if (p === wmEnums.Panels.STATUS) {
      $('#accordion-panel-status-body').removeClass("hide").addClass("show");
    } else {
      $('#accordion-panel-status-body').removeClass("show").addClass("hide");
    }

    if (p === wmEnums.Panels.CONTROLS) {
      $('#accordion-panel-controls-body').removeClass("hide").addClass("show");
    } else {
      $('#accordion-panel-controls-body').removeClass("show").addClass("hide");
    }

    if (p === wmEnums.Panels.TEMP) {
      $('#accordion-panel-temp-body').removeClass("hide").addClass("show");
    } else {
      $('#accordion-panel-temp-body').removeClass("show").addClass("hide");
    }

    if (p === wmEnums.Panels.FILES) {
      $('#accordion-panel-file-body').removeClass("hide").addClass("show");
    } else {
      $('#accordion-panel-file-body').removeClass("show").addClass("hide");
    }

    if (p === wmEnums.Panels.CONSOLE) {
      $('#accordion-panel-console-body').removeClass("hide").addClass("show");
    } else {
      $('#accordion-panel-console-body').removeClass("show").addClass("hide");
    }
  },
  SetGCommandChecksum: function SetGCommandChecksum() {
    cs = wmGCommandItem.CalcChecksum($('#text-gcommand').val());
    $('#checksum-gcommand-value').text(cs);
    jsLog.Debug("Set GCommand checksum (" + cs + ")");
  },
  SetGCommandPresetList: function SetGCommandPresetList() {
    jsLog.Verbose("Fill GCommand preset list");
    var lip = document.getElementById("list-presets").getElementsByTagName("li");
    var i = 0;

    if (lip.length === 1) {
      Object.keys(wmGCommands).forEach(function (key) {
        if (wmGCommands[key].GCode != "" && wmGCommands[key].Supported) {
          var gp = wmGCommands[key].GParams !== null ? wmTools.StringFormat(wmGCommands[key].GParams, wmGCommands[key].Values) : '';
  ...
  SetWmSettingsControls: function SetWmSettingsControls() {
    ...
    if (wmSettings.SymbolMode === 'letter') {
      ...
      $('#div-log-symbol-icon').collapse('show');
      SetConsoleSymbolIcon();
    }
  },
  SetAutotempDefault: function SetAutotempDefault() {
...
$(document).ready(function () {
  ...
  jsLog.Debug("Browser sizes: " + wmTools.GetScreenSize()); // AutoStar Actions

  WmAutostart.SetWmSettingsControls();
  WmAutostart.SetDefaultPanel();
  WmAutostart.SetGCommandPresetList();
  WmAutostart.SetAutotempDefault(); // EVENTS: Buttons

  $('#btn-clear-console').click(function () {
    WmButtons.ConsoleListClear();
  });
  $('#btn-export-console').click(function () {
    WmButtons.ConsoleListExport();
  });
  $('#btn-file-proc').click(function () {
    WmUpload.FileProcess();
  });
  $('#btn-file-proc-cancel').click(function () {
    WmUpload.Cancel();
  });
  $('#btn-file-upload').click(function () {
    WmButtons.UploadSdFile();
  });
  $('#btn-gcommand').click(function () {
    WmButtons.SendGcommand();
  });
  $('#btn-get-sdcontent').click(function () {
    WmButtons.GetSdContentList();
  });
  $('#btn-move-home-all').click(function () {
    WmButtons.SetPositionHome(this);
  });
  $('#btn-move-home-x').click(function () {
    WmButtons.SetPositionHome(this);
  });
  $('#btn-move-home-y').click(function () {
    WmButtons.SetPositionHome(this);
  });
  $('#btn-move-home-z').click(function () {
    WmButtons.SetPositionHome(this);
  });
  $('#btn-save-settings').click(function () {
    WmButtons.SaveSettings();
  });
  $('#btn-sdfile-delete-modal').click(function () {
    WmButtons.DeleteSdSelected();
  });
  $('#btn-sdfile-print-modal').click(function () {
    WmButtons.PrintSdSelected();
  });
  $('#btn-set-sdinit').click(function () {
    WmButtons.SdInit();
  });
  $('#btn-set-sdrelease').click(function () {
    WmButtons.SdRelease();
  });
  $('#btn-wsconnect').click(function () {
    WmButtons.WsConnect();
  });
  $('#btn-wsconnect-modal').click(function () {
    WmButtons.WsConnect();
  }); // EVENTS: Inputs fields

  $('#text-gcommand').keyup(function () {
    WmAutostart.SetGCommandChecksum();
  });
  $('#text-gcommand').change(function () {
    WmAutostart.SetGCommandChecksum();
  });
  $('#set-log-symbol').change(function () {
    WmConsole.SetMessageSymbol();
  });
  $('#set-log-symbol-icon').change(function () {
    WmConsole.SetSymbolIcon();
  });
  $('#file-upload').change(function () {
    WmControls.SetUpload();
  });
  $('#set-auto-temp').change(function () {
    WmControls.SetAutoTemp();
  });
  $('#auto-temp-interval').change(function () {
    WmControls.SetAutoTemp();
  });
  $('#chart-show-extruder').change(function () {
    WmCharts.Temperatures.DisplayAxis(this);
  });
  $('#chart-show-bed').change(function () {
    WmCharts.Temperatures.DisplayAxis(this);
  });
  $('#fan-speed-range').on("change", function () {
    WmControls.SetFanSpeed(this.value);
  });
  $('#set-stepper-all').change(function () {
    WmControls.SetSteppers(this);
  });
  $('#set-stepper-e').change(function () {
    WmControls.SetSteppers(this);
  });
  $('#set-stepper-x').change(function () {
    WmControls.SetSteppers(this);
  });
  $('#set-stepper-y').change(function () {
    WmControls.SetSteppers(this);
  });
  $('#set-stepper-z').change(function () {
    WmControls.SetSteppers(this);
  }); // Autorun onload

  WmConsole.Trace(new wmLogItem("Ready", wmEnums.WSMsgDirection.SENT, wmEnums.ConsoleLevels.SUCCESS));

  if (wmSettings.AutoConnect === true) {
    wmWebSoket.Connect();
  }
